...
<body>
  ...
  $localhost = "localhost/familytree";
          ...
          <form method="GET" action="#">
            <div class="input-group">
              <select class="input-group-field" name="entity">
                <?php 
                  foreach($graph->allOfType('schema:Person') as $name) {
                  echo "<option value='".$name."'>".str_replace('http://www.co-ode.org/roberts/family-tree.owl/', "", $name->get('foaf:name'))."</option>";
                }
                ?>
              </select>
      ...
      <?php
        if(isset($_GET['entity'])) {
          ...
          <div class="."callout".">
            ...
            echo "<h3>".str_replace('http://www.co-ode.org/roberts/family-tree.owl/', "", $ins->get('foaf:name'))."</h3><br/>";
            ...
            if($ins->get('foaf:name')){
              ...
              foreach ($ins->all('foaf:name') as $subject) {
            ...
            // hasChild
            if($ins->get('dbpedia-ontology:child')){
              echo "<tr><td>child</td><td>";
              foreach ($ins->all('dbpedia-ontology:child') as $subject) {
                echo '<a href="?entity='.urlencode($subject).'">'.str_replace('http://www.semanticweb.org/asus/ontologies/2016/3/ihero/', "", $subject->get('foaf:name'))."</a><br/>";
              }
              echo "</td></tr>";  
            }
